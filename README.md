<div align="center">

# 🛡️ Victor Ramalho (Kaxcav)

<a href="https://linkedin.com/in/victor-ramalho-lisboa" target="_blank">
  <img src="https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>
<a href="mailto:victor.secops@proton.me">
  <img src="https://img.shields.io/badge/-ProtonMail-8B89CC?style=for-the-badge&logo=protonmail&logoColor=white" alt="Email">
</a>

<br><br>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=58A6FF&center=true&vCenter=true&width=800&lines=🛡️+Operações+de+Segurança+(SOC)+e+Resposta+a+Incidentes;☁️+Cloud+Security+%26+Identity+(IAM);⚡+Automação+e+Detection+as+Code;🔍+Threat+Hunting+%26+Investigação+Avançada" alt="Typing SVG" />

*Defendendo infraestruturas críticas, mitigando ameaças avançadas e transformando telemetria bruta em inteligência acionável.*

</div>

---

## 🧠 Perfil Executivo

Atuo na linha de frente da defesa cibernética de **Infraestruturas Críticas do Governo Federal (Ministérios)**. Como Analista de Cibersegurança (SOC N2) e graduando em Engenharia de Software, opero na interseção entre Segurança e Engenharia: não apenas investigo incidentes de alta complexidade, mas construo as soluções. Utilizo *Detection as Code* e integrações via API para reduzir o **MTTR** e elevar a maturidade de ambientes híbridos e *Cloud Native*.

---

## 🏆 The Microsoft "Exodia" (Security Stack)
<div align="center">
  <img src="https://img.shields.io/badge/AZ--500-Azure_Security_Engineer-0078D4?style=for-the-badge&logo=microsoft" alt="AZ-500">
  <img src="https://img.shields.io/badge/SC--200-Security_Operations_Analyst-0078D4?style=for-the-badge&logo=microsoft" alt="SC-200">
  <img src="https://img.shields.io/badge/SC--300-Identity_and_Access-0078D4?style=for-the-badge&logo=microsoft" alt="SC-300">
  <img src="https://img.shields.io/badge/SC--401-Information_Protection-0078D4?style=for-the-badge&logo=microsoft" alt="SC-401">
</div>

---

## ⚙️ Arsenal de Defesa e Automação

**Automação, Cloud & Infraestrutura:** <a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=python,powershell,bash,azure,aws,kubernetes,linux,ubuntu,grafana" />
</a>

**Ecossistema de Cibersegurança:** <div align="left">
  <img src="https://img.shields.io/badge/Microsoft_Sentinel-0078D4?style=flat-square&logo=microsoftazure&logoColor=white">
  <img src="https://img.shields.io/badge/Wazuh_XDR-00A9E5?style=flat-square&logo=wazuh&logoColor=white">
  <img src="https://img.shields.io/badge/Fortinet_Ecosystem-EE3124?style=flat-square&logo=fortinet&logoColor=white">
  <img src="https://img.shields.io/badge/F5_AWAF-E32028?style=flat-square&logo=f5&logoColor=white">
  <img src="https://img.shields.io/badge/Zabbix-D40000?style=flat-square&logo=zabbix&logoColor=white">
  <img src="https://img.shields.io/badge/Trellix_EDR-000000?style=flat-square">
  <img src="https://img.shields.io/badge/Varonis_LGPD-000000?style=flat-square">
</div>

---

## 🚀 Arquitetura e Engenharia de Defesa

### 👁️ [Observabilidade SecOps Unificada (K8s + Wazuh + Grafana)](https://github.com/kaxcav0tracer/k8s-wazuh-lab)
> **Arquitetura:** Criação de um *Single Pane of Glass* correlacionando métricas de performance (Zabbix) e segurança (Wazuh) em clusters Kubernetes.  
> **Impacto:** Quebra de silos operacionais, permitindo correlacionar anomalias de hardware com alertas de invasão no OpenSearch em tempo real.

### ⚡ [Cloud SIEM & SecOps Automation Suite](https://github.com/kaxcav0tracer/secops-automation-suite-v2)
> **Arquitetura:** Laboratório SOC híbrido com Microsoft Sentinel e desenvolvimento de 6 ferramentas em PowerShell/Python (com GUI em WPF).  
> **Impacto:** Automação do fluxo de *Threat Hunting* com KQL e integração direta com APIs de Threat Intelligence (VirusTotal, AbuseIPDB).

### 🛡️ Secure Gateway & Endpoint Hardening
> **Arquitetura:** Implementação de Gateway *clientless* (Apache Guacamole) e deployment massivo de Windows LAPS.  
> **Impacto:** Eliminação de portas RDP/SSH expostas no perímetro e mitigação tática de movimentos laterais (Pass-the-Hash) no Active Directory.

---

## 📈 Telemetria de Código

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="https://github-readme-stats.vercel.app/api?username=kaxcav0tracer&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=58A6FF" width="100%" alt="GitHub Stats" />
      </td>
      <td align="center">
        <img src="https://github-readme-streak-stats.herokuapp.com/?user=kaxcav0tracer&theme=tokyonight&hide_border=true&background=0D1117&ring=58A6FF&fire=58A6FF&currStreakNum=ffffff" width="100%" alt="GitHub Streak" />
      </td>
    </tr>
  </table>
</div>

<br>
<div align="center">
  <i>"Se você faz uma tarefa manual mais de duas vezes, é hora de transformá-la em código."</i>
</div>
